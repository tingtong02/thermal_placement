# Stage 1 Tiled Matmul OS Baseline Activity Summary
...
- matrix: `MAT_DIM_I/K/J=64` in bare-metal `tiled_matmul_os.c`
...
- binary: `sim/binaries/GemminiRocketConfig/tiled_matmul_os-baremetal` (19.87 KiB)
- stdout log: `sim/logs/GemminiRocketConfig/tiled_matmul_os-baremetal.stage1_tiled_matmul_os_baseline_20260423.log` (496.00 B)
- disassembly/out log: `sim/logs/GemminiRocketConfig/tiled_matmul_os-baremetal.stage1_tiled_matmul_os_baseline_20260423.out` (327.30 MiB)
- waveform: `sim/waves/GemminiRocketConfig/tiled_matmul_os-baremetal.stage1_tiled_matmul_os_baseline_20260423.vcd` (40.83 GiB)
- signal activity CSV: `sim/activity/stage1_tiled_matmul_os_baseline_20260423_signal_activity.csv` (7.15 MiB)
- region activity CSV: `sim/activity/stage1_tiled_matmul_os_baseline_20260423_region_activity.csv` (960.00 B)
- window CSV: `sim/activity/stage1_tiled_matmul_os_baseline_20260423_window_activity.csv` (576.00 B)
...
- total_region_toggles: `3981319503`
- gemmini_target_region_toggles: `154545102`
- gemmini_target_region_toggle_share: `3.881756%`
...
- The Stage 1 trace is a full `GemminiRocketConfig` SoC/TestHarness waveform, so whole-trace top toggles are expected to be dominated by Rocket core, cache, and TileLink context.
- For the actual research target, the relevant check is whether Gemmini PE/control/datapath buckets are non-zero and internally plausible.
...
The run reached the Gemmini tiled matmul section and produced parseable RTL activity data. Stage 1 is usable for the next step because Gemmini PE/control/datapath buckets are non-zero and the selected windows are already documented in the Stage 1 window report. Interpret global whole-trace rankings only as SoC context, not as the Stage 2 implementation boundary.
