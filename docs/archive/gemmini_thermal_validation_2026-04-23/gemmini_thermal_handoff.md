...
- 已新增并跑通小 GEMM 流程：`16 x 16 x 16` Gemmini output-stationary GEMM、功能校验、完整 VCD、活动率提取、HotSpot steady-state、简单报告。
...
- [build_small_gemm_binary.sh](/home/lisihang/thermal_placement/scripts/build_small_gemm_binary.sh)
- [run_small_gemm_thermal_flow.sh](/home/lisihang/thermal_placement/scripts/run_small_gemm_thermal_flow.sh)
- [report_small_gemm_flow.py](/home/lisihang/thermal_placement/scripts/report_small_gemm_flow.py)
- [gemmini_small_gemm_thermal_report.md](/home/lisihang/thermal_placement/docs/gemmini_small_gemm_thermal_report.md)

用途：

- 运行一个真实 Gemmini `16 x 16 x 16` GEMM
- 在 bare-metal 程序中做 CPU gold 校验
- 用 Verilator debug simulator 导出完整 VCD
- 复用 activity extraction 和 HotSpot smoke 输入导出
- 生成中文简短报告
...
- 自动报告：[small_gemm_report.md](/home/lisihang/thermal_placement/reports/small_gemm/small_gemm_report.md)
...
- [run_small_gemm_thermal_flow.sh](/home/lisihang/thermal_placement/scripts/run_small_gemm_thermal_flow.sh) 已从干净 shell 跑通，并完成 `small-gemm-ok`
...
- 当前 small GEMM 的完整 VCD 约 `1.6GB`，后续多 workload 时建议优先考虑 FST、VCD 截窗或分阶段 trace
...
1. 改进 [hierarchy_map.yaml](/home/lisihang/thermal_placement/configs/gemmini/hierarchy_map.yaml)，让 `scratchpad` / `accumulator` / DMA 在 VCD 路径中更精确命中
