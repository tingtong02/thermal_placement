...
def build_config() -> dict:
    ...
    config = {
        ...
        "export_extract_rc_effort": os.environ.get("TP_STAGE2_EXPORT_EXTRACT_RC_EFFORT", "medium"),
...
def pg_diagnostic_variants(config: dict) -> list[dict[str, Any]]:
    ...
    variants = [
        ...
        {
            "name": "direction_matched_m8_m9",
            "description": "use ASAP7-preferred M9 vertical and M8 horizontal PG stripes before sroute",
            "stripe_width": width,
            "stripe_spacing": spacing,
            "stripe_distance": distance,
            "m1_over_pins": False,
            "cut_rows": False,
            "core_ring": False,
            "stripe_vertical_layer": "M9",
            "stripe_horizontal_layer": "M8",
            "core_target": "stripe",
            "block_target": "stripe",
        },
        {
...
def write_pg_diagnostic_tcl(config: dict, source_floorplan: Path, script_path: Path, variant: dict[str, Any], report_dir: Path, data_dir: Path) -> None:
    ...
    lines += [
        ...
        f"set stripe_vertical_layer {variant.get('stripe_vertical_layer', 'M8')}",
        f"set stripe_horizontal_layer {variant.get('stripe_horizontal_layer', 'M9')}",
        f"set m2_stitch_width {m2_stitch_width:.6f}",
        f"set m2_stitch_spacing {m2_stitch_spacing:.6f}",
        "puts $tp_diag \"stripe_vertical_layer=$stripe_vertical_layer\"",
